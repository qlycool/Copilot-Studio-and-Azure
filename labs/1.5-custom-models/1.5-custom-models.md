...
> **Note:** Copilot Studio includes built-in models you can select directly (such as GPT-4.1, GPT-5 Chat, GPT-5 Reasoning, Claude Sonnet, and Grok 4.1). This lab covers a **different pattern** — connecting a custom model deployed in Azure AI Foundry via its endpoint, which is useful when you need a fine-tuned or domain-specific model not available in the built-in picker.
